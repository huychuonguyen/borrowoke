package com.chuthi.borrowoke.other

import com.chuthi.borrowoke.BuildConfig

const val APP_DATABASE_NAME = "${BuildConfig.APPLICATION_ID}_db"

const val API_KEY = "6953a7421bb749c589e687746c85eee8"
const val OPENAI_API_KEY = "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"
    //"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"
//const val BASE_URL = "https://api.openai.com"
const val BASE_URL ="https://api.thedogapi.com"

// region HTTP code
const val TIME_OUT = 60L
const val NETWORK_ERROR_CODE = 19951003
const val NETWORK_ERROR_MESS = "Network connection failed"

// doggo api
const val DOGGO_API_KEY = "live_atVDl6gK4THphS192DTforoviHnKIVATf8sFzgOWAkr1uX8UvUEMtNnH9zZraLXP"

const val NOTIFICATION_GROUP_ID = 9373
const val NOTIFICATION_GROUP_KEY = BuildConfig.APPLICATION_ID

const val DEFAULT_CLICK_INTERVAL = 500L
const val NO_DELAY_CLICK_INTERVAL = 0L

const val OUTPUT_BLUR_WORKER = "OUTPUT_BLUR_WORKER"
const val INPUT_BLUR_WORKER = "INPUT_BLUR_WORKER"

// date format
const val DATE_FORMAT = "yyyy/MM/dd  HH:mm:ss"
